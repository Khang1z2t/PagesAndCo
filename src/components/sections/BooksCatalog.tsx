...
function getActiveGenre(value: string | null, availableGenres: Genre[]) {
  if (!value) {
    return undefined;
  }
...
function getActiveSort(value: string | null): BookSortValue {
...
export function BooksCatalogView({ activeGenre, activeSort, activeQuery = "" }: BooksCatalogViewProps) {
...
export function BooksCatalog() {
  ...
  const activeGenre = getActiveGenre(searchParams.get("genre"), genres);
  const activeSort = getActiveSort(searchParams.get("sort"));
  const activeQuery = getActiveQuery(searchParams.get("query"));

  return <BooksCatalogView activeGenre={activeGenre} activeSort={activeSort} activeQuery={activeQuery} />;
}
